...
class UserStatisticsHandler:
    ...
    def handle_detailed_statistics(self, call):
        ...
        - Сохраняет выбранный раздел для дальнейшей обработки (например, ввода диапазона дат).
        - Отправляет запрос на ввод диапазона дат или сообщение о том, что анализ посещённых разделов пока не реализован.
    ...
    def generate_statistics(self, chat_id, section, start_date, end_date):
        ...
        - Определяет, какой раздел статистики запрашивается: уникальные пользователи, повторные посещения или неактивные пользователи.
        - Форматирует данные в удобочитаемом виде.
        - Отправляет сообщение с результатами или уведомляет об отсутствии данных за указанный период.
        """
